...
def parse_args():
    ...
    parser.add_argument(
        '--img', type=str, default='/workspace/data/tianchi/tianchi_dataset/test1/', help='Input image file or folder path.')
    parser.add_argument(
        '--output',
        type=str,
        default='/workspace/data/drrg/result/Thrpictest1',
    ...
    parser.add_argument(
        '--det-config',
        type=str,
        default='configs/textdet/drrg/drrg_r50_fpn_unet_1200e_ctw1500.py',
        help='Path to the custom config file of the selected det model. It '
        'overrides the settings in det')
    parser.add_argument(
        '--det-ckpt',
        type=str,
        default='/workspace/data/drrg/epoch_40.pth',
    ...
    def single_pp(self, result, model):
        for arr, output, export, res in zip(self.args.arrays, self.args.output,
            ...
            if output or self.args.imshow:
                res_img = model.show_result(arr, res, out_file=output,bbox_color='red',thickness=2,score_thr=0.5)
                if self.args.imshow:
                    mmcv.imshow(res_img, 'inference results')
